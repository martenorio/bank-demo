...
function App() {
  const [dataChuck, setDataChuck] = useState([])
  const [prankChuck, setPrankChuck] = useState([])
  const getPrank =  () => {
    const options = {
      method: 'GET',
      headers: {
    ...
    fetch('https://matchilling-chuck-norris-jokes-v1.p.rapidapi.com/jokes/random', options)
      ...
      .then(response => {console.log(response); setPrankChuck(response)})
  ...
  const getPeliData = () =>{
    ...
    fetch('https://matchilling-chuck-norris-jokes-v1.p.rapidapi.com/jokes/categories', options)
      .then(response => response.json())
      .then(response => {
        console.log(response); 
        setDataChuck(response)
      })
      .catch(err => console.error(err));
  }
  useEffect(() => {
    ...
    <>
      ...
      <main className='max-w-7xl mx-auto px-4 sm:px-6 md:px-8 flex'>
        <div className=" overflow-y-auto px-4 fixed border-4 border-dashed border-gray-200 w-[300px] hidden lg:block h-[calc(100vh-83.96px)]">
          <Modal data={dataChuck} prank={prankChuck}/>
          {[1, 1, 1, 1, 1, 1,].
            map((v, i) => (
              <li className="mt-12 lg:mt-8 list-none" key={i}>
                <h5 className="mb-8 lg:mb-3 font-semibold text-slate-800" _msthash="309909" _msttexthash="324753">Personalización</h5>
                <ul className="space-y-6 lg:space-y-2 border-l border-slate-200 ">
                  <li>
                    <a className="block border-l pl-4 -ml-px border-transparent hover:border-slate-400  text-slate-600 hover:text-slate-900" href="/docs/configuration" >
                      Configuración
                    </a>
                  </li>
                  <li>
                    <a className="block border-l pl-4 -ml-px border-transparent hover:border-slate-400  text-slate-600 hover:text-slate-900  " href="/docs/content-configuration">
                      Contenido
                    </a>
                  </li>
                  <li>
                    <a className="block border-l pl-4 -ml-px border-transparent hover:border-slate-400    text-slate-600 hover:text-slate-900  " href="/docs/theme" >
                      Tema
                    </a>
                  </li>
                  <li data-active="true"><a className="block border-l pl-4 -ml-px text-sky-500 border-current font-semibold dark:text-sky-400" href="/docs/screens" >
                    Pantallas
                  </a>
                  </li>
                  <li>
                    <a className="block border-l pl-4 -ml-px border-transparent hover:border-slate-400    text-slate-600 hover:text-slate-900  " href="/docs/customizing-colors" >
                      Colores
                    </a>
                  </li>
                  <li>
                    <a className="block border-l pl-4 -ml-px border-transparent hover:border-slate-400    text-slate-600 hover:text-slate-900  " href="/docs/customizing-spacing">
                      Espaciamiento
                    </a>
                  </li>
                  <li>
                    <a className="block border-l pl-4 -ml-px border-transparent hover:border-slate-400    text-slate-600 hover:text-slate-900  " href="/docs/plugins">
                      Plugins
                    </a>
                  </li>
                  <li>
                    <a className="block border-l pl-4 -ml-px border-transparent hover:border-slate-400    text-slate-600 hover:text-slate-900  " href="/docs/presets">
                      Presets
                    </a>
                  </li>
                </ul>
